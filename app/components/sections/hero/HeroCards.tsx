...
const ICON_CLASS = 'w-7 h-7 md:w-8 md:h-8 text-terracotta-light';
  ...
  return (
    <div className="mt-4 md:mt-5 grid grid-cols-1 sm:grid-cols-3 gap-3 md:gap-4 max-w-[520px]">
      {CARDS.map((card, index) => (
        <div
          key={card.title}
          className={`flex flex-col items-center text-center gap-1.5 rounded-xl border border-cream/25 bg-navy-dark/55 backdrop-blur-md px-3 py-3.5 md:py-4 transition-all duration-300 hover:-translate-y-1 hover:shadow-soft hover:border-terracotta-light/60 animate-fade-up ${
            index === 0
        ...
        >
          {card.icon}
          <h3 className="font-display uppercase tracking-[0.08em] text-base md:text-lg text-terracotta-light mt-1">
            {card.title}
          </h3>
